def main
    ...
    a_size.times do |i|
        ...
        max = a_size * 2
        while true
            if rui[max] - rui[min] >= c
                max -= (max - min) / 2
            else
                max += (a_size * 2 - max) / 2
            end
            break if rui[max] - rui[min] >= c && rui[max - 1] - rui[min] < c
            
        end
        a_ind_x << rui[max] - rui[min]
    end
    a_ind << m if a_ind.last != m
    a_ind.unshift(0) if a_ind.first != 0
    a_ind_x << a_ind_x[0]
    a_ind_x << a_ind_x[1]
    ans = 0
    a_ind.each_cons(2).with_index do |i, ind|
        ans += (i[1]-i[0]) * a_ind_x[ind]
    end
    puts ans
end
