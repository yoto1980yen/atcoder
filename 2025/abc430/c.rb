def main
    ...
    ss = Array.new(s.size + 1) {Array.new(2, 0)}
    ss[0] = [0, 0]
    (0...s.size).each do |i|
        ss[i+1] = ss[i].dup
        ss[i+1][1] = ss[i][1] + 1 if s[i] == "b"
        ss[i+1][0] = ss[i][0] + 1 if s[i] == "a"
    end
    start = 0
    last = 0
    ans = 0
    ...
    while true do
        ...
        pp "#{start} #{last}"
        while last <= n
            break if ss[last][1] - ss[start][1] >= b
            last += 1
        end
        last = n if last > n
        pp "#{start} #{last}"
        pp ss
        pp ans += last - start + 1 - a
        last += 1
    end
    puts ans
end
