def main
    ...
    count = 2 ** n
    list = []
    if k.to_f / count.to_f == k / count
        list = Array.new(count, k / count)
        puts 0
        puts list.join(" ")
    elsif k < count
        list = Array.new(count, 0)
        k.times do |i|
            list[(count.to_f / k.to_f * i).ceil] = 1
        end
        puts 1
        puts list.join(" ")
    else
        amari = k - (k / count * count)
        list = Array.new(count, k / count)
        amari.times do |i|
            list[(count.to_f / amari.to_f * i).ceil] = 1 + k / count
        end
        puts 1
        puts list.join(" ")
    end
