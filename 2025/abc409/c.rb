def main
    ...
    map = {0 => [1]}
    index = 0
    d.each.with_index do |i, v|
        index = (index + i) % l
        map[index] ? map[index] << v + 2 : map[index] = [v + 2]
    end
    ans = 0
    (l / 3).times do |i|
        next if map[i] == nil || map[i + l / 3] == nil || map[i + l / 3 * 2] == nil
        ans += map[i].size * map[i + l / 3].size * map[i + l / 3 * 2].size
    end
    puts ans
end
