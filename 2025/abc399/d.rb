def main
    int.times do |i|
        ...
        a.each.with_index do |v, i|
            map[v] ? map[v] << i : map[v] = [i]
        end
        count = 0
        n.times do |i|
            next if a[i * 2] == a[i * 2 + 1]
            count += 1
            watch = a[i * 2 + 1]
            map[a[i * 2]][1], map[a[i * 2 + 1]][0] = map[a[i * 2 + 1]][0],  map[a[i * 2]][1]
            a[i * 2 + 1],  a[map[a[i * 2 + 1]][0]] = a[map[a[i * 2 + 1]][0]], a[i * 2 + 1]
            pp map
            pp map[watch].sort!
            if (map[watch].first - map[watch].last).abs == 1 && map[watch].first / 2 != map[watch].last / 2
                map[a[map[watch][1] - 2]].sort!
                map[watch][1], map[a[map[watch][1] - 2]][map[a[map[watch][1] - 2]].index(map[watch].last - 2)] = map[a[map[watch][1] - 2]][map[a[map[watch][1] - 2]].index(map[watch].last - 2)],  map[watch][1]
                a.swap(map[watch][1] - 2, map[watch][1])
                pp map
                count += 1
            end
        end
