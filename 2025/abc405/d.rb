def main
    ...
    h.times do |i|
        map << strary
        map[i].each.with_index do |j, index|
            if j == "E"
                start << [i, index]
            end
        end
    ...
    ways = [[0, 1, "W"], [0, -1, "E"], [1, 0, "N"], [-1, 0, "S"]]
    queue = start
    while queue.size != 0
        now = queue.shift
        ways.each do |way|
            next if now[0] + way[0] >= h || now[0] + way[0] < 0 || now[1] + way[1] >= w || now[1] + way[1] < 0
            next if map[now[0] + way[0]][now[1] + way[1]] != "."
            if way[2] == "N"
                ww = "^"
            elsif way[2] == "S"
                ww = "v"
            ...
            map[now[0] + way[0]][now[1] + way[1]] = ww 
