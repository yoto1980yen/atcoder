def main
    ...
    ans = 0
    a.each do |i|
        kyoukai = b.bsearch_index{ |x| x > i}
        if kyoukai == nil
            ans += ((i * (b.size)) - (s[-1] - s[0])).abs % 998244353
        else
            ans += ((i * (kyoukai)) - (s[kyoukai] - s[0])).abs % 998244353
            ans += ((i * (b.size - kyoukai)) - (s[-1] - s[kyoukai])).abs % 998244353
        end
    end
    puts ans % 998244353
end
