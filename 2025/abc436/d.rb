def main
    ...
    h.times do |hh|
        w.times do |ww|
            next if map[hh][ww] == "." || map[hh][ww] == "#"
            atoz[map[hh][ww]] == nil ? atoz[map[hh][ww]] = [[hh, ww]] : atoz[map[hh][ww]] << [hh, ww]
        end
    end
    visited = []
    h.times do |i|
        visited << Array.new(w, false)
    end
    queue = [[0, 0]]
    ...
    while queue.size != 0
        ...
        visi = []
        ways.each do |way|
            next if y + way[0] >= h || y + way[0] < 0 || x + way[1] >= w || x + way[1] < 0
            next if map[y + way[0]][x + way[1]] == "#" || visited[y + way[0]][x + way[1]] != false
            visited[y + way[0]][x + way[1]] = visited[y][x] + 1
            unless map[y + way[0]][x + way[1]] == "." || map[y + way[0]][x + way[1]] == "#"
                next if visi.include?(map[y + way[0]][x + way[1]])
                visi << map[y + way[0]][x + way[1]]
                queue << [y + way[0], x + way[1]]
            else
                queue << [y + way[0], x + way[1]]
            end
        ...
        unless map[y][x] == "." || map[y][x] == "#"
            atoz[map[y][x]].each do |az|
                next if visited[az[0]][az[1]] != false
                visited[az[0]][az[1]] = visited[y][x] + 1
                queue << [az[0], az[1]]
            end
            atoz[map[y][x]] = []
        end
    ...
    if visited[h - 1][w - 1] != false
        puts visited[h - 1][w - 1]
    else
        puts -1
    end
end
