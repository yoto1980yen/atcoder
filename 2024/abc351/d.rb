...
def saiki(i, j)
    $now += 1
    $way.each do |way|
        next if way[0] + i == $h || way[0] + i == -1
        next if way[1] + j == $w || way[1] + j == -1
        next if $visited[i + way[0]][j + way[1]] == $count
        return if $map[i + way[0]][j + way[1]] == "#"
    end
    $way.each do |way|
        next if way[0] + i == $h || way[0] + i == -1
        next if way[1] + j == $w || way[1] + j == -1
        next if $visited[i + way[0]][j + way[1]] == $count
        $visited[i + way[0]][j + way[1]] = $count
        saiki(i + way[0], j + way[1])
    end
end
