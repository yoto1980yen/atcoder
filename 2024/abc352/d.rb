def main
    ...
    (2..(n-k+1)).each do |i|
        ...
        if map[i - 1] == max
            ...
            # pp motu.bsearch_index {|x| x >= map[i - 1] }
            # pp motu
            motu.delete_at(motu.bsearch_index {|x| x >= map[i - 1] })
        end
        # pp motu
        if map[i + k - 1] > max
            motu.push(map[i + k - 1])
            ...
            motu.unshift(map[i + k - 1])
            min = motu.first
        else
            # pp motu.bsearch_index {|x| x >= map[i - 1] }
            motu.insert(motu.bsearch_index {|x| x >= map[i - 1] }, map[i + k - 1])
        end
        ans = [ans, max - min].min
    end
    pp ans
