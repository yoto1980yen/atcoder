def main
    ...
    c = c.reverse
    # 01の場合リバース
    zerore = Array.new(s.size + 1)
    zerore[0] = 0
    (0...s.size).each do |i|
        if i % 2 == 1
            if s[i] == 0
                zerore[i+1] = zerore[i] + c[i]
            else
                zerore[i+1] = zerore[i]
            end
        else
    ...
    # 10の場合リバース
    itire = Array.new(s.size + 1)
    itire[0] = 0
    (0...s.size).each do |i|
        if i % 2 == 1
            if s[i] == 1
                itire[i+1] = itire[i] + c[i]
            else
                itire[i+1] = itire[i]
            end
        else
    ...
    if n % 2 == 0
        ...
        (n - 1).times do |i|
            # pp "#{i + 1} ~ 0 + #{n - 1 - i} ~ 0"
            # pp zero[i + 1] - zero[0] + zerore[n - 1 - i] - zerore[0]
            ans =  [zero[i + 1] - zero[0] + zerore[n - 1 - i] - zerore[0], ans].min
        end
        # 10の場合の最小値
        (n - 1).times do |i|
            # pp "#{i + 1} ~ 0 + #{n - 1 - i} ~ 0"
            # pp iti[i + 1] - iti[0] + itire[n - 1 - i] - itire[0]
            ans =  [iti[i + 1] - iti[0] + itire[n - 1 - i] - itire[0], ans].min
        end
    else
        # 01の場合の最小値
        n.times do |i|
            # pp "#{i + 1} ~ 0 + #{n - 1 - i} ~ 0"
            # pp zero[i + 1] - zero[0] + itire[n - 1 - i] - itire[0]
            ans =  [zero[i + 1] - zero[0] + itire[n - 1 - i] - itire[0], ans].min
        end
        # 10の場合の最小値
        n.times do |i|
            # pp "#{i + 1} ~ 0 + #{n - 1 - i} ~ 0"
            # pp iti[i + 1] - iti[0] + zerore[n - 1 - i] - zerore[0]
            ans =  [iti[i + 1] - iti[0] + zerore[n - 1 - i] - zerore[0], ans].min
        end
    end
    puts ans
end
