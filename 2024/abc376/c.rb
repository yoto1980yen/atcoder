def main
    ...
    count = 0
    visited = Set.new
    patarn = [[-1, -2], [-2, -1], [1, 2], [2, 1], [1, -2], [2, -1], [-1, 2], [-2, 1]]
    m.times do |i|
        a, b = intary
        # 8パターン
        visited.add((a) * (10 ** 10) + (b))
        count += 1
        patarn.each do |xy|
            next if a + xy[0] > n || a + xy[0] < 1 || b + xy[1] > n || b + xy[1] < 1
            unless visited.include?((a + xy[0]) * (10 ** 10) + (b + xy[1]))
                visited.add((a + xy[0]) * (10 ** 10) + (b + xy[1]))
                # count += 1
                # pp "#{a + xy[0]} #{b + xy[1]}"
            end
        end
