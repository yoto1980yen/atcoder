def main
    ...
    q.times do |i|
        r = 0
        b, k = intary
        nowindex = a.bsearch_index {|x| x >= b}
        nowindex = n - 1 if nowindex == nil
        # leftとright出にブタン
        min = nowindex
        max = nowindex + k
        max = n - 1 if max >= n
        while min + 1 == max
            center = (min + max) / 2
            right = b - nowindex[center]
            if nowindex - k - (center - nowindex) < 0
                min = center
                next
            end
            if a[nowindex - k - (center - nowindex) + 1] >= right
                max = center
            else
                min = center
            end
        ...
        pp b - [a[nowindex + min + 1], a[nowindex - k - (min + 1)]].min
