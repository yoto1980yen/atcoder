def main
    ...
    while querys.size != 0
        query = querys.shift
        ways.each do |i|
            next if query[0] + i[0] >= h || query[0] + i[0] < 0 || query[1] + i[1] >= w || query[1] + i[1] < 0
            if map[query[0] + i[0]][query[1] + i[1]] != "#" && visited[query[0] + i[0]][query[1] + i[1]] == false
                visited[query[0] + i[0]][query[1] + i[1]] = true
                if query[2] - 1 >= 1
                    querys << [query[0] + i[0], query[1] + i[1], query[2] - 1]
                end
            end
        end
    end
