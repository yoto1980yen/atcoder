def main
    ...
    n.times do |i|
        map[a[i]] = [a[i - 1], a[i + 1]]
        map[a[i]][1] = nil if i == n - 1
        map[a[i]][0] = nil if i == 0
    end
    start = a[0]
    int.times do |i|
        q = intary
        if q[0] == 1
            map[q[2]] = [q[1], map[q[1]][1]]
            map[q[1]] = [map[q[1]][0], q[2]]
        else
            map[map[q[1]][0]][1] = map[q[1]][1] unless map[q[1]][0] == nil
            map[map[q[1]][1]][0] = map[q[1]][0] unless map[q[1]][1] == nil
            start = map[q[1]][1] if q[1] == start
            map.delete(q[1])
        end
    end
    ans = []
    map.keys.size.times do |i|
        ans << start
        start = map[start][1]
    end
    puts ans.join(" ")
