def main
    ...
    n.times do |i|
        # i行の物がなくなるまでループし運ぶ
        # 奥から持って帰る(途中積めるなら積む)
        while w[i].count(0) != 20
            indeies = []
            taikyu = []
            w[i].reverse.each.with_index do |v, ii|
                if v == 0
                elsif indeies == []
                    indeies << n - ii - 1
                    taikyu << d[i][n - ii - 1]
                    ...
                    judge = true
                    taikyu.each do |t|
                        judge = false if (n - ii - 1 + 1 + i) * v >= t
                    end
                    if judge == true
                        indeies << n - ii - 1
                        newtaikyu = []
                        taikyu.each do |t|
                            newtaikyu << t - (n - ii - 1 + 1 + i) * v
                        end
                        newtaikyu << d[i][n - ii - 1]
                        taikyu = newtaikyu.dup
                    end
                end
            end
            indeies.each do |ii|
            ...
            # 物をはしまで回収する
            indeies.first.times do |ii|
                ans << "R"
            end
            ans << "1"
            modori = indeies.shift
            # 戻りながら回収
            modori.times do |ii|
                ans << "L"
                if indeies.size >= 1
                    if indeies.first == modori - ii - 1
                        ans << "1" 
                        indeies.shift
                    end
                end
