def main
    ...
    ans[0].each.with_index do |i, inde|
        # 3つ分の分裂期を処理装置間に置く
        if inde == ans[0].size - 1
            route.last[2] = ans[0][-2]
            route.last[3] = ans[0][-2]
        else
            inx = (processor_positions[i][0] - start[0]) / 3
            iny = (processor_positions[i][1] - start[1]) / 3
            3.times do |xy|
    ...
    hazureroute = []
    route.each_slice(3).with_index do |i, inde|
        dist_sq = sorter_positions.map.with_index do |(x, y), i|
        ...
        dist_sq.each do |j|
            ...
            next if belt_cross?(belt, i[2][0], sorter_positions[j[1]]) == true
            next if belt_cross?(belt, processor_positions[ans[0][inde]], sorter_positions[j[1]]) == true
            belt << [i[1][0], sorter_positions[j[1]]]
            belt << [i[2][0], sorter_positions[j[1]]]
            belt << [processor_positions[ans[0][inde]], sorter_positions[j[1]]]
            hazureroute << [sorter_positions[j[1]], j[1], ans[0][inde], i[1][3], ans[0][inde]]
            route[inde * 3 + 1][2] = j[1]
            used << i[1]
            break
        end
        if hazureroute.size != inde + 1
            i.each.with_index do |p, pi|
                ...
                belt << [processor_positions[ans[0][inde]], i[1][0]]
                route[inde * 3 + pi][2] = ans[0][inde] - n
                break
            end
        end
    end
    ...
    route.each.with_index do |i, ii|
        ans[2 + i[1]] = [kps[i[4]][0], i[2] + n, i[3] + n]
    end
