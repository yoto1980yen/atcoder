def main
    ...
    scores = 0
    n.times do |i|
        field << Array.new(n) { Array.new(3, false)}
    ...
    (t-1).times do |i|
        ...
        # Level1の領地があるなら優先して埋める
        # 初めの30ターン
        if i <= 30
            can_visit.each do |v|
                if v[1][1] > 0 && v[1][2] < 2# そうでないなら自分の領地以外を拡張するがレベルが高い場合はパス
                    next_position = v[0]
                    break
                # 自分の領地且つレベルが1ならレベル2に補強する(u != 1の場合のみ)
                elsif v[1][1] == 0 && v[1][2] == 1 && u != 1
                    next_position = v[0]
                    break
                end
            end
            ...
            can_visit.each do |v|
                if v[1][1] > 0 && v[1][2] < 2# そうでないなら自分の領地以外を拡張するがレベルが高い場合はパス
                    next_position = v[0]
                    break
                # 自分の領地の価値が平均より高いならもっと補強  && v[1][0] >= scores_ave
                elsif v[1][1] == 0 && v[1][2] < u
                    next_position = v[0]
                    break
                end
            end
        ...
        puts "# #{next_position} #{new_can_visit}"
        # next_positionが決まらなかった場合
        if next_position == []
            new_can_visit.each do |v|
                if v[1][1] == -1# 自分の領地以外を拡張する
                    next_position = v[0]
                    break
                end
                next_position = v[0]
