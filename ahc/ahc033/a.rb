        ...
        while nowsize <= 10000
            ...
            5.times do |i|
                (1..4).each do |j|
                    if map[i][j] == want
                        # 右端が運ぶ奴なら子クレーンに運ばせる
                        if map[i].slice(j..).uniq.size == 1 && map[i].slice(j..).uniq.first == want
                            ...
                            owaricount += 1
                            # 次に探すコンテナを+1
                            findlist[runsuu][0] = nextfindlist[runsuu].shift
                            nextfindlist[runsuu].push[30]
                        # 5個あるときは右にずらす
                        elsif zan[i] == 5
            ...
            if zancount == 4
                (0..4).each do |k|
                    temp = findlist[k][0]
                    findlist[k][0] = nextfindlist[k][0]
                    nextfindlist[k][0] = temp
                end
                zancount = 0
                pp "#sdad"
                pp findlist
