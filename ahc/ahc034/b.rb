def main
    ...
    plus = []
    minas = []
    #4つの区画に分けて積もっている順に埋めていく
    jun = jun.sort_by {|x| -x[1]}
    jun.each do |i|
        if i[1] >= 0
            plus << i[0]
        else
    ...
    ansdousa = []
    # 前半戦
    while Time.now - start_time <= 0.8
        nowplus = plus.shuffle
        nowcost = 0
        x = 0
        dump = 0
        y = 0
    ...
    # 後半戦
    usiroansdousa = []
    while Time.now - start_time <= 1.9
        nowplus = minas.shuffle
        nowcost = 0
        x = xx
        dump = dumpp
        y = yy
        ...
        if nowcost <= cost
            usiroansdousa = dousa.dup
        end
    end
    ansdousa.each do |i|
        puts i
    end
    usiroansdousa.each do |i|
        puts i
    end
end
