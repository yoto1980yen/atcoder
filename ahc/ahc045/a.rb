def main
    ...
    n, m, q, l, w = intary
    g = intary
    gsort = g.each.with_index.sort.to_a.reverse
    citys = []
    city_center = [] # 中心座標
    n.times do |i|
        citys << intary
        city_center << [(citys.last[0] + citys.last[1]) / 2, (citys.last[2] + citys.last[3]) / 2]
    end
    city_center.each.with_index.sort
    # 出来るだけ50都市ずつで縦に分けたい 50超えるならそこまで拡張
    groups = [0]
    nowcount = 0
    gsort.each do |i|
        nowcount += i[0]
    ...
    group_index = 0
    gg = groups.dup
    city_center.each.with_index.sort.each do |i|
        groups_city[-1] << i
        group_index += 1
        if group_index == gg.first
            gg.shift
            group_index = 0
            groups_city << []
        end
    ...
    gg = gsort.dup
    realgroup = [[]]
    groups_city.each do |c|
        c.sort_by{|v, i| v[1]}.each do |i|
            realgroup[-1] << i
            if realgroup[-1].size == gg.first[0]
                gg.shift
                realgroup << []
            end
        end
    ...
    realgroup.pop
    # 占い師にq回よさげな道路を聞く
    realgroups_load = Array.new(m) {Array.new}
    realgroup.each.with_index do |i, index|
        if i.size == 2
            realgroups_load[index] << [i.first[1], i.last[1]]
        elsif i.size >= 3
            uranai = []
            i.each do |j|
                uranai << j[1]
                if uranai.size == l
                    puts "? #{uranai.size} #{uranai.join(" ")}"
                    puts "\n"
                    STDOUT.flush
                    (uranai.size - 1).times do |t|
                        realgroups_load[index] << intary
                    end
                    uranai = [uranai.last]
                end
            end
            if uranai.size == 2
                realgroups_load[index] << [i[-2][1], i[-1][1]]
            elsif uranai.size >= 3
                puts "? #{uranai.size} #{uranai.join(" ")}"
                puts "\n"
                STDOUT.flush
        ...
        realgroups_load[index] = realgroups_load[index].dup.uniq
    end

    ansmap = {}
    m.times do |i|
        ansmap[gsort[i][1]] = [realgroup[i], realgroups_load[i]]
    end
    # 答えを出力
    puts "!"
    m.times do |i|
        cc = []
        ansmap[i][0].each do |j|
            cc << j[1]
        end
        puts cc.join(" ")
        ansmap[i][1].each do |j|
            puts j.join(" ")
        end
    end
    puts "\n"
