def main
    ...
    a = []
    n.times do |i|
        a << intary.each_with_index.sort.reverse
    end
    env = [0, 1, 10, 20, 30, 40]
    real_ans = Array.new(100000, 0)
    env.each do |per|
        n.times do |ii|
            ans = []
            h = hh.dup
            ...
            count = 1
            while h.max >= 1
                if wepons.size >= 1 # 武器がある場合は優先して使う
                    percent = rand(1..100)
                    a[wepons[0][0]].each do |i| # 攻撃力の高い順に箱を攻撃
                        next if h[i[1]] <= 0 # 既にあけられていたらスルー
                        next if per >= percent
                        wepons[0][1].times do
                            h[i[1]] -= i[0]
                            wepons[0][1] -= 1
                    ...
                    if wepons[0][1] >= 1 # まだ武器の耐久値が残っているなら後ろに回す
                        wepons.push(wepons.shift) 
                    else # 武器の耐久値がなくなったら捨てる
                        wepons.shift
                    end
                    
                else # 武器がなくなったら一番耐久値の低い箱を素手で開ける
                    tar = h.max
                    h.each do |i|
                        tar = [i, tar].min if i >= 1
                    end
                    target = h.index(tar)
                    h[target].times do |i|
                        ans << [-1, target]
            ...
            break if Time.now - start_time >= 1.7
        end
    end
    real_ans.each do |i|
        puts i.join(" ")
