  ...
  n.times do |i|
    n.times do |j|
      ...
      when 0 # identity
        ii = i
        jj = j
      when 1 # flip left-right
        ii = i
        jj = n - 1 - j
      when 2 # transpose
        ii = j
        jj = i
      when 3 # transpose + flip left-right (on transformed grid)
        ii = n - 1 - j
        jj = i
      else
        ii = i
...
def main
  ...
  [0, 1, 2, 3].each do |mode|
    ta = transform_grid(a, mode)
    res = build_requested_path(n, ta)
    unless res[:ok]
      fail_logs << "mode=#{mode} fail: #{res[:reason]} (#{res[:path].size}/#{n * n})"
      next
    end

    path_orig = res[:path].map { |i, j| inverse_point(i, j, n, mode) }
    next unless valid_path?(path_orig, n)

    sc = score_path(path_orig, a)
  ...
  if best_path.nil?
    fail_logs.each { |s| warn s }
    warn "all 4 patterns failed"
    exit 1
  end

  best_path.each { |i, j| puts "#{i} #{j}" }
end
