def main
    ...
    $uv.each do |i|
        ...
        alist.each_with_index.sort.each do |j|
            $newuv[i[0]] ? $newuv[i[0]] << i[1][j[1]] : $newuv[i[0]] = [i[1][j[1]]]
        end
    end
    $uv = $newuv
    pp $uv
    ...
    g = []
    # まずは6個以上つなげる
    $a.each_with_index.sort.reverse.each do |a|
        next if $visited[a[1]]
        $wantsize = a[0]
        $visited[a[1]] = true
        nowlist = saiki(a[1], [a[1]])
        if nowlist.size >= 10
            g << nowlist
        else
            nowlist.each do |i|
    ...
    # 出力の形に変形
    p = Array.new(1000, -1)
    g.each.with_index do |i, index|
        p[i.first] = -1
        (1...i.size).each do |j|
            p[i[i.size - j]] = i[i.size - j + 1]
        end
    end
    puts p.join(" ")
end
...
#----------------------------------------------------------------------------------
require "set"
def saiki(index, list)
    return if list.size == 10
    $uv[index].each do |i|
        next if $visited[i] || $wantsize <= $a[i]
        $visited[i] = true
        list << i
        saiki(i, list)
        return list
