def main
    ...
    sweet = [0]
    sakujo = []
    map.sort!.each.with_index do |i, j|
        next if i[0] == 0
        if sweet.last != i[0]
            ans << [sweet.last, 0, i[0], 0]
            unless mapset[i[0]]
                mapset[i[0]] = RBTree.new
                mapset[i[0]][0] = true
            end
            sakujo << j if i[1] == 0
            sweet << i[0]
    ...
    soda.each.with_index do |i, v|
        now = [0,0,i[0],i[1]]
        cost = 1000000000000
        search_index = sweet.bsearch_index{ |k| k >= i[0] }
        (0...(search_index+1)).each do |j|
            nowcost = i[0] - sweet[j] + i[1] - mapset[sweet[j]].upper_bound(i[1])[0]
            if cost >= nowcost
                now = [sweet[j], mapset[sweet[j]].upper_bound(i[1])[0], i[0], i[1]]
                cost = nowcost
            end
        end
        costmax += cost
        ans << now
        mapset[now[2]][now[3]] = true
    end
    # i[1] version
    costmaxx = 0
    mapset = {0 => RBTree.new}
    ...
    sweet = [0]
    sakujo = []
    map.sort{|a,b| a[1] <=> b[1]}.each.with_index do |i, j|
        next if i[1] == 0
        if sweet.last != i[1]
            anss << [0, sweet.last, 0, i[1]]
            unless mapset[i[1]]
                mapset[i[1]] = RBTree.new
                mapset[i[1]][0] = true
            end
            sakujo << j if i[1] == 0
            sweet << i[1]
    ...
    soda.each.with_index do |i, v|
        now = [0,0,i[0],i[1]]
        cost = 1000000000000
        search_index = sweet.bsearch_index{ |k| k >= i[1] }
        (0...(search_index+1)).each do |j|
            nowcost = i[0] - mapset[sweet[j]].upper_bound(i[0])[0] + i[1] - sweet[j]
            if cost >= nowcost
                now = [mapset[sweet[j]].upper_bound(i[0])[0], sweet[j], i[0], i[1]]
                cost = nowcost
            end
        end
        costmaxx += cost
        anss << now
        mapset[now[3]][now[2]] = true
    end
    if costmaxx < costmax
        puts anss.size
        anss.each do |i|
