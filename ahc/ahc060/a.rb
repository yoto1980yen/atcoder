def main
    ...
    $real_ans = []
    $pattern = 0
    while Time.now - start_time <= 1.8
        $red_map = Array.new($n, false)
        $rumdom_count = 1
        $change_rumdom = rand(1...30)
        $before_position = -1
        $now_position = 0
        $ans = []
        $t.times do |time|
            $rumdom_count = rand(10...15)
            # 用意したランダム回分ランダムに移動する
            $rumdom_count.times do |i|
                # 3%の確率でストロベリーにする
                if rand(0...1000) <= $change_rumdom && $now_position >= $k && $red_map[$now_position] == false
                    $ans << -1
                    $red_map[$now_position] = true
                    next
                end
                while true
                    now_rundom = rand(0...$map[$now_position].size)
                    next if $map[$now_position][now_rundom] == $before_position
                    $before_position = $now_position
                    $now_position = $map[$now_position][now_rundom]
                    $ans << $now_position
                    break
                end
            end
            # 現在地からランダムにアイスクリームショップに行く
            queue = [[$now_position, $before_position]]
            visited = Array.new($n, false)
            visited[$now_position] = []
            while queue.size != 0
                now, before = queue.shift
                if now < $k
                    $go = visited[now]
                    break
                end
                $map[now].each do |i|
                    next if visited[i] != false
                    next if i == before
                    visited[i] = visited[now].dup
                    visited[i] << i
                    queue << [i, now]
                end
            end
            ...
            if $go.size == 1
                ...
                $now_position = $go.last
                $before_position = $go[-2]
            end
            break if $ans.size >= 10000
        end
        cap = []
        $ice_pattern = Array.new($k) {Array.new}
        $red_map = Array.new($n, "w")
        $ans.each.with_index do |i, v|
            if i >= 10
                cap << $red_map[i]
            elsif i == -1
                $red_map[$ans[v - 1]] = "r"
            else
                $ice_pattern[i] << cap.join("")
                cap = []
            end
        end
        now_pattern = 0
        $ice_pattern.each do |i|
            now_pattern += i.uniq.size
        end
    ...
    $real_ans.slice(0, 10000).each do |a|
        puts a
    end
