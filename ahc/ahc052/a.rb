def main
    ...
    $n, $m, $k = intary
    $lobo = []
    $mapp = Array.new($n) { Array.new($n, 0)}
    $m.times do |i|
        $lobo << intary
        $mapp[$lobo.last[0]][$lobo.last[1]] = 1
    end
    $yokokabe = []
    $tatekabe = []
    ($n).times do
        $tatekabe << intary_no_space
    end
    ($n-1).times do
        $yokokabe << intary_no_space
    end
    # controller
    $way_distination = {"L" => [0, -1], "R" => [0, 1], "D" => [1, 0], "U" => [-1, 0] }
    $ways = ["U", "D", "R", "L"]
    $c = []
    $k.times do |i|
        $c << Array.new($n, "U") if i == 0
        $c << Array.new($n, "D") if i == 1
        $c << Array.new($n, "R") if i == 2
        $c << Array.new($n, "L") if i == 3
        $c << Array.new($n, "U") if i >= 4
    end
    turn = 0
    $ans = []
    $maxx = 10000
    # 0の掃除機で幅優先で0を埋めていく
    while Time.now - start_time <= 1.7
        $map = []
        $mapp.each do |i|
            $map << i.dup
        end
        ...
        while turn < $n ** 2 * 2
            ...
            now_ans << $ways[x]
            $m.times do |m|
                next if $lobo[m][0] + $way_distination[$c[x][m]][0] < 0 || $lobo[m][0] + $way_distination[$c[x][m]][0] >= $n || $lobo[m][1] + $way_distination[$c[x][m]][1] < 0 || $lobo[m][1] + $way_distination[$c[x][m]][1] >= $n
                if $way_distination[$c[x][m]][0] == 1
                    next if $yokokabe[$lobo[m][0]][$lobo[m][1]] == 1
                elsif $way_distination[$c[x][m]][0] == -1
                    next if $yokokabe[$lobo[m][0] - 1][$lobo[m][1]] == 1
                elsif $way_distination[$c[x][m]][1] == 1
                    next if $tatekabe[$lobo[m][0]][$lobo[m][1]] == 1
                elsif $way_distination[$c[x][m]][1] == -1
                    next if $tatekabe[$lobo[m][0]][$lobo[m][1] - 1] == 1
                end
                $lobo[m][0] += $way_distination[$c[x][m]][0]
                $lobo[m][1] += $way_distination[$c[x][m]][1]
                max -= 1 if $map[$lobo[m][0]][$lobo[m][1]] == 0
                $map[$lobo[m][0]][$lobo[m][1]] = 1
            end
            if max == 0
                $c.each do |i|
                    puts i.join(" ")
