def main
    ...
    dlist.each do |i|
        ...
        weight = 0
        sabun = 0
        sabun = count - 1998 if count >= 1998
        nowcount = 0
        while sabun != 0 do
            if weightlist[nowcount] >= 2 
                weightlist[nowcount] -= 1
                sabun -= 1
            end
            nowcount = ((nowcount + 1) % n)
        end
        omoi = weightlist.each_with_index.sort.reverse
        map = {}
        uenow = 0
        sitanow = 0
        weigthlimit = w
        limit = 0
        omoi.each.with_index do |i, iindex|
            if i[0] >= weigthlimit
                limit = weigthlimit - (i[0] + 1) / 2
                map[i[1]] = [limit, 0, weigthlimit, w]
                weigthlimit = limit
            elsif weigthlimit - uenow >= i[0]
                map[i[1]] = [uenow, 0, uenow + i[0], 500]
                uenow += i[0]
            else
                if sitanow + i[0] >= weigthlimit - (omoi.size - iindex)
                    map[i[1]] = [sitanow, 500, weigthlimit - (omoi.size - iindex), w]
                    sitanow = weigthlimit - (omoi.size - iindex)
                else
                    map[i[1]] = [sitanow, 500, sitanow + i[0], w]
                    sitanow += i[0]
                end
