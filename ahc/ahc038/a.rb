def main
    ...
    smap = []
    tmap = []
    n.times {smap << gets.chomp.split("").map(&:to_i)}
    n.times {tmap << gets.chomp.split("").map(&:to_i)}
    n.times do |i|
        n.times do |j|
    ...
    100000.times do |i|
        ...
        queue = []
        way.each do |i|
            queue << [y + i[1], x + i[0]] if x + i[0] >= 0 && if x + i[0] < n &&  y + i[1] >= 0 && if y + i[1] < n
        end
        # 幅優先探索
