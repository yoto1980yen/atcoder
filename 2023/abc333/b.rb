def main
    ...
    t1, t2 = strary
    atoe = ("A".."E").to_a
    # list = {"A" => ["E", "B"], "B" => ["A", "C"], "C" => ["E", "B"], "A" => ["E", "B"], "A" => ["E", "B"]}
    s = 0
    t = 0
    s = 1 if atoe[(atoe.index(s1) - 1) % 5] == s2 || atoe[(atoe.index(s1) + 1) % 5] == s2
    t = 1 if atoe[(atoe.index(t1) - 1) % 5] == t2 || atoe[(atoe.index(t1) + 1) % 5] == t2
    if s == t
        puts "Yes"
    else
        puts "No"
