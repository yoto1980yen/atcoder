def main
    ...
    match = []
    (n - 1).times do |i|
        match << i + 1 if s[i] == s[i + 1]
    end
    match_reverse = match.reverse.dup
    q.times do |i|
        l, r = intary
        left = match.bsearch_index {|x| x >= l }
        right = match.bsearch_index {|x| x > r - 1 }
        left = match.size if left == nil
        right = match.size if right == nil
        puts right - left
    end
end
