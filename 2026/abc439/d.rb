def main
    ...
    list.each do |i|
        ...
        five  = i[1][1]
        seven = i[1][2]
        next if three == [] || five == [] || seven == []
        five.each do |f|
            tmin = three.bsearch_index{|t| t > f} || 0
            smin = seven.bsearch_index{|s| s > f} || 0
            ans += (three.size - tmin) * (seven.size - smin)
            tmax = three.reverse.bsearch_index{|t| t < f} || 0
            smax = seven.reverse.bsearch_index{|s| s < f} || 0
            ans += tmax * smax
        end
    end
    puts ans
