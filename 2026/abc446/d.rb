def main
    ...
    a.sort.uniq.each do |i|
        map[i].each do |m|
            ...
            genzaiti = m[0]
            while true
                break if map[i + index] == nil
                now = map[i + index].bsearch_index{|x| x[0] > genzaiti}
                break if now == nil
                break if map[i + index][now][1] == 1
                genzaiti = map[i + index][now][0]
                map[i + index][now][1] = 1
                now_ans += 1
                index += 1
            end
