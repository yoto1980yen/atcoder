def main
    ...
    q.times do |i|
        x, y = intary
        pp min_index = a.bsearch_index{|xx| xx >= x}
        if min_index == nil
            puts x + y - 1
            next
        end
        ...
        while min + 1 != max do
            center = (min + max) / 2
            max_index = a.bsearch_index{|xx| xx >= center}
            if max_index == nil
                max_index = n
            end
            max = center - (max_index - min_index)
            if max < x + y + (max_index - min_index) - 1
                min = center
            else
                max = center
            end
            pp "#{min} #{max} #{center} #{x + y + (max_index - min_index) - 1}"
        end
        puts center
    end
end
