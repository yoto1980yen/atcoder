def main
    ...
    h.times do |i|
        w.times do |j|
            next if visited[i][j] == true || s[i][j] == "#"
            judge = true
            visited[i][j] = true
            queue = [[i, j]]
            while queue.size != 0
                y,x = queue.shift
                ways.each do |way|
                    if y + way[0] <= 0 || y + way[0] >= h - 1 || x + way[1] <= 0 || x + way[1] >= w - 1
                        if y + way[0] < 0 || y + way[0] >= h || x + way[1] < 0 || x + way[1] >= w
                            judge = false
                            next
                        end
                        judge = false if s[y + way[0]][x + way[1]] == "."
                    end
                    if s[y + way[0]][x + way[1]] == "." && visited[y + way[0]][x + way[1]] == false
